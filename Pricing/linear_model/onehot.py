...
#drop negative corr with saleprice
train_all = pd.concat([train_df_munged,label_df],axis=1)
corr = train_all.corr()
drop_columns = list(corr['SalePrice'].loc[corr['SalePrice'] < 0,].index)
train_df_munged.drop(drop_columns,axis=1,inplace=True)
test_df_munged.drop(drop_columns,axis=1,inplace=True)
...
def elasticnet():
    regr = Elasticnet_regression(train_df_munged, label_df['SalePrice'])
    # Run prediction on training set to get a rough idea of how well it does.
    y_pred = regr.predict(train_df_munged)
    y_test = label_df
    print(" score on training set: ", rmse(y_test, y_pred))
    # Run prediction on the Kaggle test set.
    y_pred_elastic = regr.predict(test_df_munged)
    ################################################################################
    # Blend the results of the two regressors and save the prediction to a CSV file.
    y_pred = np.exp(y_pred_elastic)
    pred_df = pd.DataFrame(y_pred, index=test_df["Id"], columns=["SalePrice"])
    pred_df.to_csv('../../submission1227.csv', header=True, index_label='Id')

def GBDT_model():
    model = GDBT_regression(train_df_munged, label_df['SalePrice'])
    y_pred = model.predict(test_df_munged)
    y_pred = np.exp(y_pred)
    pred_df = pd.DataFrame(y_pred, index=test_df["Id"], columns=["SalePrice"])
    pred_df.to_csv('../../submission1227.csv', header=True, index_label='Id')

def model_average():
    X_train,X_test,Y_train,Y_test = train_test_split(train_df_munged,label_df['SalePrice'],test_size=0.3,random_state=0)
    elas = Elasticnet_regression(X_train,Y_train)
    # gbdt = GDBT_regression(train_df_munged,label_df['SalePrice'])
    elas_predict = elas.predict(X_test)
    # gbdt_result = gbdt.predict(X_test)
    # pred = (elas_predict + gbdt_result)/2
    print('rmse value:',rmse(Y_test,elas_predict))
